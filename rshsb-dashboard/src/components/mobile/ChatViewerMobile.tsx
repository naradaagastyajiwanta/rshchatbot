...
export default function ChatViewerMobile() {
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      .channel('user_profiles_changes_mobile')
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  const handleUserSelect = (waNumber: string) => {
    ...
    setShowChat(true); // Navigate to chat view when user is selected
  };

  const handleBackToUsers = () => {
    setShowChat(false); // Navigate back to user list
  };
  ...
  // WhatsApp-like UX: Show either user list or chat, not both at once
  if (showChat && selectedUser) {
    return (
      <div className="flex flex-col h-full bg-white rounded-lg shadow overflow-hidden">
        {/* Mobile chat header with back button */}
        <div className="bg-[#8e003b] text-white p-2 flex items-center">
          <button 
            onClick={handleBackToUsers}
            className="p-2 rounded-full hover:bg-[#7b0033] transition-colors"
          >
          ...
          <h2 className="ml-2 font-medium">
            {users.find(user => user.wa_number === selectedUser)?.name || selectedUser}
          </h2>
        </div>
